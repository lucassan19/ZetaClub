...
exports.login = async (req, res) => {
  ...
  try {
    ...
    console.log('Body recebido:', { username, passwordLength: password?.length });
    console.log('Esperado do .env:', { envUser, envPassLength: envPass?.length });
    ...
    if (isMatch) {
      const token = jwt.sign(
        { role: 'admin' }, 
        process.env.JWT_SECRET || 'zetaclub_secret_fallback', 
        { expiresIn: '8h' }
      );
