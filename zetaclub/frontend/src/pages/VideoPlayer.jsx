  ...
  useEffect(() => {
    const fetchVideo = async () => {
      try {
        ...
        setDislikesCount(res.data.dislikes || 0);
    ...
    <div className="max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Link
            to="/"
            className="inline-flex items-center gap-2 text-slate-400 hover:text-white transition group mb-2"
          >
            <ArrowLeft
              size={18}
            ...
            {isVideoLoading && (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-slate-950">
                <div className="relative">
                  ...
                  <Play
                    className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-primary-500"
                    size={24}
                    fill="currentColor"
                  />
                </div>
            ...
            {qualityLevels.length > 0 && (
              <div className="absolute top-6 right-6 z-20">
                <button
                  ...
                  className="p-3 bg-black/60 hover:bg-primary-600 rounded-2xl text-white transition-all backdrop-blur-md border border-white/10 shadow-xl"
                >
                  <Settings size={20} />
                </button>

                {showSettings && (
                  <div className="absolute right-0 mt-3 w-56 bg-slate-900/95 border border-white/10 rounded-2xl shadow-2xl py-3 backdrop-blur-xl">
                    ...
                    <button
                      ...
                      className={`w-full text-left px-5 py-3 text-sm font-bold hover:bg-primary-600 transition-colors ${
                        currentQuality === -1
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-white/5">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl transition-all font-bold text-sm border active:scale-95 ${
                      liked
                        ? "bg-primary-600 border-primary-500 text-white shadow-lg shadow-primary-900/40"
                        : "bg-slate-700/50 hover:bg-slate-700 border-white/5"
                    }`}
                  >
                    <ThumbsUp size={18} />
              ...
              <div className="space-y-4">
                ...
                <p className="text-slate-300 leading-relaxed whitespace-pre-wrap font-medium">
                  {video.description ||
                    "Este vídeo não possui uma descrição detalhada."}
                </p>
              </div>
            </div>
          </div>
